class QuestionAnswerTool {
  ...
  constructor({ data }: { data: { items: { question: string; answer: string }[] } }) {
    this.data = { items: [{ question: "Exemple", answer: "Exemple" }] };
    this.wrapper = null;
  }

  // Метод для рендеринга всего компонента
  render() {
    if (this.wrapper) {
      this.wrapper.innerHTML = ""; // Удаляем старый контент
    } else {
      this.wrapper = document.createElement("div");
      this.wrapper.classList.add("question-answer-tool");
      this.wrapper.style.border = "1px solid #ddd";
      this.wrapper.style.padding = "10px";
      this.wrapper.style.marginBottom = "10px";
    }

    // Рендерим все элементы из массива
    this.data.items.forEach((item, index) => {
      this.addItem(this.wrapper as HTMLElement, item, index);
    });

    // Добавляем кнопку для добавления нового элемента
    ...
    addButton.addEventListener("click", () => {
      ...
      this.render(); // Принудительный ререндеринг
    });
  ...
  // Рендеринг отдельного вопроса-ответа
  private addItem(wrapper: HTMLElement, item: { question: string; answer: string }, index: number) {
