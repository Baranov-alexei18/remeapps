class TextColorTool {
  ...
  render() {
    ...
    textColorButton.title = 'Выбор цвета текста';
  ...
  surround(range: Range) {
    ...
    // Проверяем, есть ли у выделенного текста текущие стили
    const existingNode = range.commonAncestorContainer.parentElement as HTMLElement;
    const currentBackground = existingNode?.style.backgroundColor;

    const span = document.createElement('span');
    span.style.color = this.selectedColor;
    span.style.backgroundColor = currentBackground; // Сохраняем текущий фон
